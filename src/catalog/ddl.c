...
static ObjectAddress o_define_relation(CreateStmt *cstmt,
									   const char *queryString);
	...
	else if (IsA(pstmt->utilityStmt, CreateStmt))
	{
		...
		while (stmts != NIL)
		{
			...
			if (IsA(stmt, CreateStmt))
			{
				...
				o_define_relation(cstmt, queryString);
			}
...
o_define_relation(CreateStmt *cstmt, const char *queryString)
{
	...
	address = DefineRelation(cstmt, RELKIND_RELATION, InvalidOid, NULL,
							 queryString);
...
o_create_ctas_internal(List *attrList, IntoClause *into)
{
	...
	intoRelationAddr = o_define_relation(cstmt, NULL);
