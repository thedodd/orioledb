...
orioledb_parallelscan_initialize(Relation rel, ParallelTableScanDesc pscan)
{
	...
	SpinLockInit(&poscan->intpage_access);
	SpinLockInit(&poscan->worker_start);
	SpinLockInit(&poscan->intpage_loading);
	clear_fixed_key(&poscan->prevHikey.fixed);
	poscan->int_page[0].loaded=false;
	poscan->single_leaf_page_rel = false;
	poscan->offset = 0;
	poscan->leader_started = false;
	poscan->cur_int_pageno = 0;
	poscan->int_finish = false;
	poscan->first_page_loaded = false;
	memset(poscan->int_page[0].img, 0, ORIOLEDB_BLCKSZ);
	memset(poscan->worker_active, 0, sizeof(poscan->worker_active));

//	elog(DEBUG1, ">> %s", PG_FUNCNAME_MACRO);
	Assert(is_orioledb_rel(rel));
